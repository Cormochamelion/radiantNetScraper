...
(define-public python-pipe
  (package
    ...
    (source
     (origin
       (method url-fetch)
       (uri (pypi-uri "pipe" version))
       (sha256
        (base32 "0393970599dsrz1dp7bsna1qbkkbb1v268m4y3x2ym5wwfc329ba"))))
    (build-system pyproject-build-system)
    ;; In theory the package uses pytest, but it is not specified in
    ;; pyproject.toml. Manually specifying `#:test-backend "pytest"` results
    ;; in `%exception #<&test-system-not-found>` being raised. Don't understand
    ;; why.
    (arguments (list #:tests? #f))
    (home-page "https://github.com/JulienPalard/Pipe")
    (synopsis "Module enabling a sh like infix syntax (using pipes)")
    (description "Module enabling a sh like infix syntax (using pipes).")
    (license expat)))

(define %packages
  (append (list python-pipe)
          (map specification->package
            (list "python-beautifulsoup4"
                  "python-dotenv"
                  "python-numpy"
                  "python-pandas"
                  ;; 2.32.3 causes issues with finding certificates.
...
(packages->manifest
 (append (map specification->package (list "python" "python-pip"))
